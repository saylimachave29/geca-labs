...
-- Puzzle A: Finding the staff member assigned to both "sibling" projects
SELECT s.name
FROM staff s
...
WHERE p1.proj_id < p2.proj_id             -- Ensure they are two different projects
  AND p1.dept_id = p2.dept_id             -- Same home department
  AND SUBSTRING(p1.title, 1, 4) = SUBSTRING(p2.title, 1, 4); -- Shared name prefix
